"""-------------------------------------------------------------
    Script Name:   snappy_bgz_step_clean.py
    Description:   SNAP processing chain for VV/VH S1 coherence data - step clean
    Created By:    Chris Moore
    Date Created:  19.02.21
    Date Modified: 01.06.23
-------------------------------------------------------------"""

# step clean: Delete processing files from previous step and temporary files
...
if 12 in clean_steps:
    # if have stack
    if os.path.exists(io_base + '_stack.dim'):
        os.remove(io_base + '_stack.dim')
        shutil.rmtree(io_base + '_stack.data', ignore_errors=True)
...
if 13 in clean_steps:
    ...
    for pol in ['VH', 'VV']:
        # if have stack
        if os.path.exists(io_base + '_stack_' + pol + '_BM' + '.dim'):
            os.remove(io_base + '_stack_' + pol + '_BM' + '.dim')
            shutil.rmtree(io_base + '_stack_' + pol + '_BM' + '.data', ignore_errors=True)
